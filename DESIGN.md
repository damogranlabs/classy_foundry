...
- **Sketcher** (a sketch step) — *implemented*. Click places a point via
  ...
  - **Reference-point reuse** *(implemented; live ref, revised from snapshot)* — in point mode a
    ...
    reference** to it (pick-based, via `ps.pick` → step); a click that misses drops a free
    coordinate on the work plane. So a sketch vertex *is* the named point — the sketch follows it
    when the curve/param changes, and codegen names it (`cb.MappedSketch([leading_edge, …], quads)`
    — the readable, parametric output the user wanted). This works because `positions` is already a
    `point_list` (reference-or-literal per entry, exactly like `Face` corners), so `build`/codegen
    needed no change; only the sketcher's *display* did — every place it reads vertex coordinates
    now goes through `sketch.resolved_positions(context)` (a ref → its built coordinate, a literal
    through; an unbuilt ref → None, skipped). A stored entry is therefore a `PointStep` *or* an
    `[x, y, z]`, mixed freely in one sketch.
